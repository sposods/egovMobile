...
<html>
	<head>
		...
		<script type="text/javaScript" language="javascript">
			...
			function showView(expenseId, othbcAtConfirm) {
				if(othbcAtConfirm == "N"){
		...
		<div id="list" data-role="page">
			...
			<div data-role="content">
				...
				<ul data-role="listview" data-count-theme="d"> 
					...
					<c:choose>
						<c:when test="${empty edmManageList}">
							<li class="com-egovNodata">
		               			<spring:message code="common.nodata.msg"/>
		               		</li>			
						</c:when>
						<c:otherwise>
							<c:forEach var="edmManage" items="${edmManageList}">
								<li>
									<a href="javascript:showView('${edmManage.expenseId}', 'Y')" data-ajax="false">
										<h3> ${edmManage.categoryNm } </h3>
										<p><span class="uss-txtBlue">${edmManage.amount }</span><span class="uss-txtBlack">${edmManage.mberNm }</span><span class="uss-txtDate">${fn:substring(edmManage.updtDt, 0, 10)}</span></p>
										<span class="ui-li-count">${edmManage.expenseDt}</span>
									</a> 
								</li>
							</c:forEach>					
						</c:otherwise>
