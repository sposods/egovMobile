 ...
 * 수정일 			수정자 		수정내용
 * ---------- --------- ------------------- 
 * 2024.05.02. 우시재 		NSR 보안조치 ( 멀티미디어 제목에 XSS 방지 구현 추가 )
 */ 
var resultList;
var fileInfoList;

// 20240502 NSR 보안조치(멀티미디어 제목에 XSS 방지 구현 추가)
...
function fn_egov_initl_mobilemultimedialist() {
	...
	$.getJSON(contextPath + "/mbl/com/mlt/selectMobileMultimediaList.mdo", function(json) {
		...
		for (var i = 0; i < resultList.length; i++) {
			...
			// 20240502 NSR 보안조치(멀티미디어 제목에 XSS 방지 구현 추가)
			list_html += "<h3>" + fn_egov_clearxss_minimum(result.mltmdSj) + " ( " + result.mltmdNm + " ) </h3><p>" + result.creatDt + "</p>";
