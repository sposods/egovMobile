 ...
 * 수정일 			수정자 		수정내용
 * ---------- --------- ------------------- 
 * 2024.05.02. 우시재 		NSR 보안조치 ( 사진 제목에 XSS 방지 구현 추가 )
 */
var totcnt;
var imgNum;
var resultList;
...
function fn_egov_clearxss_minimum(str){
...
function fn_egov_initl_mobilephotolist(){
	$.getJSON(contextPath + "/mbl/com/mpa/selectMobilePhotoList.mdo", function(json) {
		...
		for (var i = 0; i < totcnt; i++) {
			...
			// 20240502 NSR 보안조치 (사진 제목에 XSS 방지 구현 추가 )
			list_html += "<li><img alt=\"" + fn_egov_clearxss_minimum(result.photoSj) + "\" title =\""+ fn_egov_clearxss_minimum(result.photoSj) + "\" src='" + contextPath + "/cmm/fms/getImage.mdo?atchField=" + result.archField + "&fileSn=0/'></li>";
			list_html += "</a>";
		}
		detail_html += "<li>";
