 ...
 *   2017.03.06	       조성원	      시큐어코딩(ES)-시스템 데이터 정보노출[CWE-497]
...
public class EgovFileMntrg extends Thread {
	...
	ArrayList<Object> realOriginalList = new ArrayList<Object>(); // 최초의 원본리스트
	ArrayList<Object> originalList = new ArrayList<Object>(); // 직전리스트는 주기적으로 직전목록정보로 갱신된다.
	ArrayList<Object> currentList = new ArrayList<Object>(); // 직전리스트와 비교할 현시점 리스트
	ArrayList<Object> changedList = new ArrayList<Object>(); // 직전리스트와 비교한 시점에 발생된 변경리스트
	ArrayList<Object> totalChangedList = new ArrayList<Object>(); // 최초리스트와 비교한 변경 리스트
	...
	protected EgovFileMntrg(String filename, File logFile) {
		...
		originalList = new ArrayList<Object>(realOriginalList);
		writeLog("START");
		setDaemon(true);
		checkAndConfigure();
	...
	protected void doOnChange(ArrayList<Object> changedList) {
		//log.debug("doOnChange() start");
		for (int i = 0; i < changedList.size(); i++) {
			writeLog((String) changedList.get(i));
		}
		changedList.clear(); //직전리스트와 비교해서 변경된 내역은 로그처리한 후 초기화한다.
		originalList = new ArrayList<Object>(currentList); //현재리스트가 직전리스트가 된다.(새로 생성해야 함!)
		cnt++;
			...
			*/
			...
			for (int i = 0; i < currentList.size(); i++) {
				for (int j = 0; j < originalList.size(); j++) {
					if (((String) currentList.get(i)).equals(originalList.get(j))) {
						isSame = true;
					}
	...
	public void run() {
		while (!interrupted) {
			...
			//2017.03.06	       조성원	      시큐어코딩(ES)-시스템 데이터 정보노출[CWE-497]
				...
				LOGGER.error("[InterruptedException] Try/Catch...Interrupted Runing : "+ e.getMessage());
			}catch (Exception e) {
				LOGGER.error("["+e.getClass()+"] Try/Catch...Runing : " + e.getMessage());
			}
			checkAndConfigure();
