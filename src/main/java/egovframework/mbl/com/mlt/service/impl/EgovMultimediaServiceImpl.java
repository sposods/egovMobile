...
/**
 * 개요 - 멀티미디어 제어에 대한 Service Interface를 구현한다.
 *
 * 상세내용 - 멀티미디어에 대한 등록, 수정, 삭제, 조회 기능을 제공한다. - 멀티미디어에 대한 조회기능은 목록, 상세조회로 구분된다.
 * 
 * @author 정홍규
 * @since 2011.08.23
 * @version 1.0
 ...
 *   2024.05.02. 김수용          NSR 보안조치 (업로드 파일의 확장자 검사 메소드 구현 추가)
 * 
 *      </pre>
 */

@Service("MultimediaService")
public class EgovMultimediaServiceImpl extends EgovAbstractServiceImpl implements EgovMultimediaService {
	...
	public Multimedia selectMultimedia(Multimedia multimedia) throws Exception {
		Multimedia ret = multimediaDAO.selectMultimedia(multimedia);
		return ret;
	}
	...
	public void copyFileToRelativePath(List<FileVO> fileList) throws Exception {
		...
		absolutePath = currPath.substring(0, currPath.lastIndexOf("WEB-INF")) + "multimedia";
		...
		for (int i = 0; i < fileList.size(); i++) {
			File sFile = new File(fileList.get(i).getFileStreCours(), fileList.get(i).getStreFileNm());
			File dFile = new File(absolutePath, fileList.get(i).getStreFileNm() + "." + fileList.get(i).getFileExtsn());
	 ...
	 */
	@Override
	public List<MultimediaFileInfo> getMultimediaFileInfoFromXML() throws Exception {
		String XMLPath = this.getClass().getResource("").getPath() + "MultimediaFileInfo.xml";
	...
	public String getBrowserInfoFromXML(String mltmdNm, List<String> extList) throws Exception {
		String browserInfo = "";
		String mltmdNmType = "";
	 ...
	 */
	@Override
	public boolean checkMultimediaFileExtension(List<FileVO> fileList) throws Exception {
		
		List<String> whiteListExtensions = new ArrayList<>();
		String XMLPath = this.getClass().getResource("").getPath() + "MultimediaFileInfo.xml";
		SAXBuilder builder = new SAXBuilder();
		Document document = builder.build(new File(XMLPath));
		Element rootElement = document.getRootElement();
		List<Element> multimediaFileInfoElements = rootElement.getChildren();
		for (Element multimediaFileInfoElement : multimediaFileInfoElements) {
			whiteListExtensions.add(multimediaFileInfoElement.getAttributeValue("extension"));
		}
		for (int i = 0; i < fileList.size(); i++) {
			String fileExt = fileList.get(i).getFileExtsn();
